...
this.addEventListener("message", function (e) {
    ...
    if (
        ("status" === params[0] && postMessage(testStatus + ";" + dlStatus + ";" + ulStatus + ";" + pingStatus + ";" + clientIp + ";" + jitterStatus + ";" + dlProgress + ";" + ulProgress + ";" + pingProgress),
        ...
        try {
            ...
            for (var key in s) void 0 !== settings[key] ? (settings[key] = s[key]) : twarn("Unknown setting ignored: " + key);
            ...
            /Edge.(\d+\.\d+)/i.test(ua) && (settings.forceIE11Workaround = !0),
                ...
                void 0 !== s.telemetry_level && (settings.telemetry_level = "basic" === s.telemetry_level ? 1 : "full" === s.telemetry_level ? 2 : 0),
    ...
    "abort" === params[0] &&
        (tlog("manually aborted"), clearRequests(), (runNextTest = null), interval && clearInterval(interval), 1 < settings.telemetry_level && sendTelemetry(), (testStatus = 5), (jitterStatus = pingStatus = ulStatus = dlStatus = ""));
});
var ipCalled = !1;
function getIp(done) {
    tlog("getIp"),
        ipCalled ||
            ...
            xhr.open(
                "GET",
                settings.url_getIp + url_sep(settings.url_getIp) + (settings.getIp_ispInfo ? "isp=true" + (settings.getIp_ispInfo_distance ? "&distance=" + settings.getIp_ispInfo_distance + "&" : "&") : "&") + "r=" + Math.random(),
                !0
            ),
            xhr.send());
}
    ...
    if ((tlog("dlTest"), !dlCalled)) {
                            ...
                            if (1 === testStatus) {
                                ...
                                (xhr[i] = x),
                                    (xhr[i].onprogress = function (event) {
                                        if ((tlog("dl stream progress event " + i + " " + event.loaded), 1 !== testStatus))
                                            try {
                                                x.abort();
                                            } catch (e) {}
                                        var loadDiff = event.loaded <= 0 ? 0 : event.loaded - prevLoaded;
                                        isNaN(loadDiff) || !isFinite(loadDiff) || loadDiff < 0 || ((totLoaded += loadDiff), (prevLoaded = event.loaded));
                                    }.bind(this)),
        ...
        interval = setInterval(
            function () {
                ...
                (graceTimeDone && (dlProgress = t / (1e3 * settings.time_dl)), t < 200) ||
                    (graceTimeDone
                        ? ((dlStatus = ((8 * (totLoaded / (t / 1e3)) * settings.overheadCompensationFactor) / (settings.useMebibits ? 1048576 : 1e6)).toFixed(0)),
                          ((t / 1e3 > settings.time_dl && 0 < dlStatus) || failed) &&
                              ((failed || isNaN(dlStatus)) && (dlStatus = "Fail"), clearRequests(), clearInterval(interval), (dlProgress = 1), tlog("dlTest finished " + dlStatus), done()))
                        : t > 1e3 * settings.time_dlGraceTime && (0 < totLoaded && ((startT = new Date().getTime()), (totLoaded = 0)), (graceTimeDone = !0)));
            }.bind(this),
            200
        );
    }
...
function ulTest(done) {
    if ((tlog("ulTest"), !ulCalled)) {
        ...
        var req = [],
            reqsmall = [];
        for (i = 0; i < settings.xhr_ul_blob_megabytes; i++) req.push(r);
        (req = new Blob(req)), (r = new ArrayBuffer(262144));
        try {
            r = new Uint32Array(r);
            for (i = 0; i < r.length; i++) r[i] = Math.random() * maxInt;
        } catch (e) {}
                ...
                function () {
                    if (3 === testStatus) {
                        ...
                        var ie11workaround,
                            prevLoaded = 0,
                            x = new XMLHttpRequest();
                        if (((xhr[i] = x), settings.forceIE11Workaround)) ie11workaround = !0;
                        else
                            try {
                        ...
                        ie11workaround
                            ? ((xhr[i].onload = function () {
                              ...
                              xhr[i].open("POST", settings.url_ul + url_sep(settings.url_ul) + "r=" + Math.random(), !0),
                              xhr[i].setRequestHeader("Content-Encoding", "identity"),
                              xhr[i].send(reqsmall))
                            : ((xhr[i].upload.onprogress = function (event) {
                                  if ((tlog("ul stream progress event " + i + " " + event.loaded), 3 !== testStatus))
                                      try {
                                          x.abort();
                                      } catch (e) {}
                                  var loadDiff = event.loaded <= 0 ? 0 : event.loaded - prevLoaded;
        ...
        for (i = 0; i < settings.xhr_ulMultistream; i++) testStream(i, settings.xhr_multistreamDelay * i);
        interval = setInterval(
            function () {
                tlog("UL: " + ulStatus + (graceTimeDone ? "" : " (in grace time)"));
                var t = new Date().getTime() - startT;
                (graceTimeDone && (ulProgress = t / (1e3 * settings.time_ul)), t < 200) ||
                    (graceTimeDone
                        ? ((ulStatus = ((8 * (totLoaded / (t / 1e3)) * settings.overheadCompensationFactor) / (settings.useMebibits ? 1048576 : 1e6)).toFixed(2)),
                          ((t / 1e3 > settings.time_ul && 0 < ulStatus) || failed) &&
                              ((failed || isNaN(ulStatus)) && (ulStatus = "Fail"), clearRequests(), clearInterval(interval), (ulProgress = 1), tlog("ulTest finished " + ulStatus), done()))
                        : t > 1e3 * settings.time_ulGraceTime && (0 < totLoaded && ((startT = new Date().getTime()), (totLoaded = 0)), (graceTimeDone = !0)));
            }.bind(this),
            200
        );
    }
            ...
            tlog("ping"),
                ...
                (xhr[0].onload = function () {
                    if ((tlog("pong"), 0 === i)) prevT = new Date().getTime();
                        ...
                        1 === i ? (ping = instspd) : ((ping = 0.9 * ping + 0.1 * instspd), (jitter = jitter < instjitter ? 0.2 * jitter + 0.8 * instjitter : 0.9 * jitter + 0.1 * instjitter)), (prevInstspd = instspd);
                    }
                ...
                (xhr[0].onerror = function () {
                    tlog("ping failed"),
                        0 === settings.xhr_ignoreErrors && ((jitterStatus = pingStatus = "Fail"), clearRequests(), done()),
                        1 === settings.xhr_ignoreErrors && doPing(),
                        2 === settings.xhr_ignoreErrors && (++i < settings.count_ping ? doPing() : done());
                }.bind(this)),
                xhr[0].open("GET", settings.url_ping + url_sep(settings.url_ping) + "r=" + Math.random(), !0),
                xhr[0].send();
        }.bind(this);
        doPing();
    }
...
function sendTelemetry() {
    if (!(settings.telemetry_level < 1)) {
        ...
        try {
            var fd = new FormData();
            fd.append("dl", dlStatus), fd.append("ul", ulStatus), fd.append("ping", pingStatus), fd.append("jitter", jitterStatus), fd.append("log", 1 < settings.telemetry_level ? log : ""), xhr.send(fd);
        } catch (ex) {
            var postData =
                "dl=" +
                ...
                "&log=" +
                encodeURIComponent(1 < settings.telemetry_level ? log : "");
